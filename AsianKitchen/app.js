...
//Showing all foods when page was loaded!
document.addEventListener("DOMContentLoaded", listFoods);

//Listing foods according to their category!
const listKoreanFoods = () => {
  let allKoreanFoods = "";

  menu.map(item => {
    if(item.category === "Korea"){
      allKoreanFoods += createFood(item);
    }
  })

  menuSection.innerHTML = allKoreanFoods;
}
...
const listJapanFoods = () => {
  ...
  menu.map(item => {
    if(item.category === "Japan"){
      allJapanFoods += createFood(item);
    }
  })

  menuSection.innerHTML = allJapanFoods;
}
...
const listChinaFood = () => {
  ...
  menu.map(item => {
    if(item.category === "China"){
      allChinaFoods += createFood(item);
    }
  })

  menuSection.innerHTML = allChinaFoods;
}
...
document.querySelector("#china").addEventListener("click", listChinaFood);
